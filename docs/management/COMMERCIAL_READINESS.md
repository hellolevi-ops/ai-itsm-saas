...
|---|---|---|---|---|---|
| Core ITSM | PARTIAL | Minimal ticket loop verified. | M1 ticket submit, queue, detail, messages, status transitions and E2E pass. | Service catalog, SLA and templates pending. | Yes |
...
| PLG activation | PARTIAL | Signup to first value path verified. | Register/login/create workspace, ticket submit, AI suggestion and knowledge draft path verified by E2E. | Invite/member activation loop pending. | Yes |
...
| Multi-tenant security | PARTIAL | Negative cross-tenant tests pass. | Workspace guard tests plus ticket/AI/knowledge workspace-scoped repository and service tests pass. | Full controller/integration permission matrix pending. | Yes |

...
| P2-QA-001 | P2 | OPEN | Jest config emits an ESM warning even though tests pass. | `npm test -- --runInBand` passes 141/141 but warns about loading `jest.config.ts` as ESM. | Rename config to `.mjs`/`.cjs` or set package module semantics deliberately. |
| P2-PRODUCT-001 | P2 | OPEN | Product docs use both Free/Growth/Pro and Free/Team/Growth/Business package naming. | `docs/product/MVP_SCOPE.md` mentions Free/Growth/Pro; `docs/product/BUSINESS_PLAN.md`, schema and M7 implementation use Free/Team/Growth/Business. | Confirm final package naming before production pricing/legal copy. |

...
| P2-QA-001 | P2 | OPEN | Jest config emits an ESM warning even though tests pass. | `npm test -- --runInBand` passes 135/135 but warns about loading `jest.config.ts` as ESM. | Rename config to `.mjs`/`.cjs` or set package module semantics deliberately. |
